...
export default function HeroVideo() {
  ...
  return (
    <div
      className="relative aspect-video rounded-sm overflow-hidden shadow-column border border-gold/30 bg-ink group"
    ...
    >
      ...
      {!playing && (
        ...
        >
          <span className="w-20 h-20 rounded-full bg-ink-deep/70 border-2 border-gold/70 flex items-center justify-center backdrop-blur-sm">
            <svg viewBox="0 0 24 24" className="w-9 h-9 text-gold fill-current ml-1">
      ...
      {/* Controls bar */}
      <div
        className={`absolute bottom-0 left-0 right-0 px-4 pt-6 pb-3 bg-gradient-to-t from-ink-deep/90 via-ink-deep/50 to-transparent transition-opacity duration-300 ${
      ...
      >
        ...
        >
          ...
          {/* Thumb */}
          <div
            className="absolute top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-gold shadow-md opacity-0 group-hover/bar:opacity-100 transition-opacity"
